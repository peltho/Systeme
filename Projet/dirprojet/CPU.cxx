...
CPU::CPU(const string &id, ostream  &log, istream &consoleIn, ostream &consoleOut, const int nGenReg, const int nProc)
    : 
    ...
    prReg  . setVal(PROCPSWFRAMESTART);             // registre du compteur de processus <- 0
    pcReg  . setVal(0);                             // registre du compteur de prgms <- 0
    spReg  . setVal(MEMORYSIZEPERPROCESS - 1);      // registre du pointeur de pile <- 999 octets
    mdReg  . switchToMaster();                      // passage au mode maître
    inReg0 . setVal(nProc - 1);                     // registre 0 <- nb processus - 1
...
void CPU::interrupt(const int interruptNumber, bool qCLINTInstr) throw(CExc) {
    ...
    mem[proc2stPSW] . storeAt(frameStart + wordSize,     pcReg . getVal()); // Stocke pcReg (ProgramCounter renvoie l'instruction executée)
    ...
    pcReg . setVal(mem[KERNELPID] . loadFrom(interruptNumber * wordSize));  // L'instruction <- n° d'interruption
    dumpReg(logStream,true);
}
