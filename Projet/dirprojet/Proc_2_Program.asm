...
SETRI R0 0        ;LNR on récupère la sémaphore
SETRI R2 1        ;LNR on stocke la valeur 1 dans R2, correspond à la fois au code de P() et au nombre de semops
SETRI R3 215      ;LNR on stocke la valeur 215 (qui sera une @) dans R3
STMEM R3 R0       ;LNR on stocke la valeur 0 (index de la semaphore) à R3, donc à l'@215
ADDRG R3 R3 R2    ;LNR R3 contient l'@215+1, donc l'@216
STMEM R3 R2       ;LNR on stocke la valeur 1 (code de P()) à R3, donc à l'@216
SETRI R3 225      ;LNR on stocke la valeur 225 (qui sera une @) dans R3
STMEM R3 R0       ;LNR on stocke la valeur 0 (index de la semaphore) à R3, donc à l'@225
ADDRG R3 R3 R2    ;LNR R3 contient l'@225+1, donc l'@226
STMEM R3 R0       ;LNR on stocke la valeur 0 (code de V()) à R3, donc à l'@226
